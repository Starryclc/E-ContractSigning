...
quary: function() {
   qID= $('#qID').val();
    var template=App.info[qID];
    if(template==null){
        alert("未查询到第"+qID+"号合同！请仔细检查合同编号是否有误")
    }else {
        $("#e_contractInfo").html(template);
    }
 },
 ...
 createC: function() {
  ...
  if(checkID(idCard1)==0){
    alert('甲方身份证号码有误，请重新填写！');
    return false;
  }else if((checkID(idCard2))==0){
